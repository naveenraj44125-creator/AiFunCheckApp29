 ...
 */
async function getFeed(userId, pagination, storageInstance = InMemoryStorage_1.storage) {
    ...
    const visiblePosts = allPosts.filter(post => (0, PostService_1.canViewPost)(post, userId, storageInstance));
    // Requirement 4.4: Sort posts by creation date in descending order (newest first)
    const sortedPosts = visiblePosts.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
